...
import { Search, Loader2, FileInput, AlertCircle, CheckCircle, Copy, Trash2 } from 'lucide-react';
...
export const ImportPanel: React.FC<ImportPanelProps> = ({ foundPlayers, setFoundPlayers }) => {
  ...
  return (
    <div className="h-full flex flex-col">
      <div className="glass-panel rounded-t-xl p-4 border-b-0">
      ...
      <div className="flex-1 overflow-hidden flex flex-col glass-panel border-t-0 rounded-b-xl mt-4">
        <div className="p-3 border-b border-white/10 flex justify-between items-center">
          <h3 className="font-semibold text-white/80">Found Players ({foundPlayers.length})</h3>
          <button 
        ...
        <div className="flex-1 overflow-y-auto p-3 space-y-2">
           ...
           {foundPlayers.map(player => (
             <PlayerCard key={player.fid} player={player} />
           ))}
        </div>
      </div>
    </div>
