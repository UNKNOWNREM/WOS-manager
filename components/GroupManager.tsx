...
import { Plus, Download, GripVertical, Pencil, Trash2 } from 'lucide-react';
...
export const GroupManager: React.FC = () => {
  ...
  const exportTSV = () => {
    ...
    const headers = ['Nickname', 'FID', ...activeGroup.columns.map(c => c.name)];
    const rows = activeGroup.players.map(p => {
      const basic = [p.nickname, p.fid];
      const custom = activeGroup.columns.map(c => p.customData[c.id] || '');
      return [...basic, ...custom].join('\t');
    });
          ...
          <>
            ...
            <div className="px-4 py-3 border-b border-white/10 flex flex-wrap gap-4 justify-between items-center bg-black/20">
              <div className="flex items-center gap-2">
                 ...
                 <button 
                    onClick={() => openModal({ type: 'RENAME_GROUP', title: 'Rename Group', inputValue: activeGroup.name, targetId: activeGroup.id, confirmLabel: 'Save' })}
                    className="px-3 py-1.5 rounded-md bg-white/5 hover:bg-teal-600/30 text-xs text-white flex items-center gap-1.5 border border-white/10 transition-colors"
                 >
